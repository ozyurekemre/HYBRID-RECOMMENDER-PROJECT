...
# Step 4: # In the index, there are userIDs, movie names in the columns and ratings as values.
...
# Step 2: Create a new dataframe named user_movie_count for each user, which contains the number of movies watched by the selected user.
...
# Step 3: Create a new dataframe named top_users by filtering out users that have high correlation (over 0.65) with the selected user.
...
# Step 3: Step3: Select the films with a weighted rating greater than 3.5 in recommendation_df and rank them by weighted rating.
...
# Step 3: Filter the user_movie_df dataframe created in the User based recommendation section according to the selected movie id.movie[movie["movieId"] == movie_id]["title"].values[0]
